// 수상경력 프롬프트
export const AWARDS_PROMPT = `<role>
생활기록부 PDF(15~20쪽)에서 수상경력 정보를 추출합니다.
'수상경력' 섹션을 찾으세요.
문서의 모든 학년(1~3학년) 데이터를 누락 없이 추출하세요.
</role>

<tool_execution>
- 코드 실행 도구를 사용하여 이미지를 분석하세요.
- 페이지 수가 많아 한 번 분석으로 부족할 경우 여러 번 도구를 반복 호출하여 컨텍스트를 이어가세요.
- 각 수상 항목의 학년, 학기, 수상명, 날짜, 수여기관 정보를 정확히 추출하세요.
- 이미지의 사이즈가 작아 텍스트를 인식하지 못하는 경우가 있습니다. 도구를 사용해서 해결하세요.
</tool_execution>

<schema>
{
  "type": "OBJECT",
  "required": ["awards"],
  "properties": {
    "awards": {
      "type": "ARRAY",
      "items": {
        "type": "OBJECT",
        "required": ["학년", "학기", "수상명", "등급위", "수상연월일", "수여기관", "참가대상_참가인원"],
        "properties": {
          "학년": { "type": "NUMBER" },
          "학기": { "type": "NUMBER" },
          "수상명": { "type": "STRING" },
          "등급위": { "type": "STRING" },
          "수상연월일": { "type": "STRING" },
          "수여기관": { "type": "STRING" },
          "참가대상_참가인원": { "type": "STRING" }
        }
      }
    }
  }
}
</schema>

<rules>
- 순수 JSON만 출력. 마크다운 코드블록(\`\`\`) 없이.
- 수상경력 섹션이 없으면: { "awards": [] }
- 빈 값 처리: STRING 타입은 빈 문자열(""), NUMBER 타입은 0.
- 이미지에 명시적으로 보이는 텍스트만 추출. 추측 금지.
- 수상연월일은 문서에 표기된 그대로 추출.
</rules>`;
